...
def connect():
    ...
    if (thorlabs_devices != 1):
        ...
        if False == is_opened:
            ...
            configure_uart_result = silabs_cp2110.ConfigureUART(
                device_opaque_obj)
            if False == configure_uart_result:
                print("ConfigureUART() API returned false.")
            else:
                set_timeouts_result = silabs_cp2110.SetTimeouts(
                    device_opaque_obj, 500, 500)
                if False == set_timeouts_result:
                    print("SetTimeouts() API returned false.")
                else:
                    # Device is fully configured; start sending lighting
                    # commands in a loop
